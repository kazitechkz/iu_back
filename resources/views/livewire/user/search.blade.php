        ...
        @else
           ...
           <div class="form-group">
             <label class="block text-sm font-medium text-gray-700 dark:text-gray-400" for="user_result">{{__("table.user_id")}}</label>
                <select name="{{$user_name}}" class="form-control" id="user_result">
                @foreach($users as $userItem)
                    @if(is_object($userItem))
                        <option class="py-2 px-3 focus:outline-none all-colors ease-in-out duration-150 relative group text-secondary-600 dark:text-secondary-400 flex items-center justify-between cursor-pointer focus:bg-primary-100 focus:text-primary-800 hover:text-white dark:focus:bg-secondary-700 hover:bg-primary-500 dark:hover:bg-secondary-700" value="{{$userItem->searchable->id}}">
